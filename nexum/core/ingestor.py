...
def _load_raw(path: Path) -> dict[str, Any]:
    ...
    if suffix == ".json":
        try:
            ...
            raise NexumIngestError(f"Invalid JSON in '{path}': {exc}") from exc

    if suffix in {".yaml", ".yml"}:
        try:
            data = yaml.safe_load(text)
        except yaml.YAMLError as exc:
            raise NexumIngestError(f"Invalid YAML in '{path}': {exc}") from exc
        if not isinstance(data, dict):
            raise NexumIngestError(
...
def ingest(file_path: str | Path) -> dict[str, Any]:
    ...
    Raises:
        NexumIngestError: if the file is missing, unparseable, or of unknown format.
    """
    path = Path(file_path)

    if not path.exists():
        raise NexumIngestError(f"File not found: '{path}'")
