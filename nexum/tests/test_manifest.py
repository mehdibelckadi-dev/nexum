...
from nexum.core.scorer import ScoreResult, calculate
from nexum.manifest.generator import (
...
def _finding(severity: str, rule_id: str = "NEXUM-000", path: str = "/test", method: str = "GET") -> Finding:
    return Finding(
...
class TestManifestShape:
    ...
    def test_required_keys_present(self):
        ...
        for key in ("manifest_version", "scanned_at", "source_file",
                    "inferred_risk_tier", "nexum_risk_score",
...
class TestFindingsSummary:
    ...
    def test_summary_contains_expected_fields(self):
        f = _finding("CRITICAL", rule_id="NEXUM-001", path="/invoices", method="GET")
        summary = _manifest(findings=[f])["findings_summary"]
        assert len(summary) == 1
        entry = summary[0]
        assert entry["rule_id"] == "NEXUM-001"
    ...
    def test_summary_preserves_order(self):
        findings = [
            _finding("CRITICAL", rule_id="NEXUM-001"),
            _finding("HIGH", rule_id="NEXUM-004"),
            _finding("MEDIUM", rule_id="NEXUM-005"),
        ]
        ids = [e["rule_id"] for e in _manifest(findings=findings)["findings_summary"]]
        assert ids == ["NEXUM-001", "NEXUM-004", "NEXUM-005"]
...
class TestManifestWithFixtures:
    ...
    def test_openapi_fixture_findings_summary_has_3_entries(self):
        ...
        assert all(e["rule_id"] == "NEXUM-001" for e in m["findings_summary"])

    def test_mcp_fixture_score_40_tier_1(self):
        m = self._run(str(FIXTURES / "sample_mcp.json"))
        assert m["nexum_risk_score"] == 40
    ...
    def test_mcp_fixture_findings_summary_rule_ids(self):
        ...
        assert ids == ["NEXUM-002", "NEXUM-004", "NEXUM-005"]
