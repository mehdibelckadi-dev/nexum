"""Tests for all five PACT rule implementations and the engine."""
...
from pact.core.engine import _SEVERITY_ORDER
from pact.core.ingestor import ingest
from pact.core.rules.auth_leakage import AuthLeakageRisk
from pact.core.rules.base import Finding
from pact.core.rules.destructive_ambiguity import DestructiveAmbiguity
from pact.core.rules.unbounded_scope import UnboundedScope
from pact.core.rules.idempotency import IdempotencyMissing
from pact.core.rules.schema_volatility import SchemaVolatility
...
class TestDestructiveAmbiguity:
    ...
    def test_delete_without_path_param_is_flagged(self):
        ...
        assert findings[0].rule_id == "PACT-002"
...
class TestUnboundedScope:
    ...
    def test_delete_without_param_or_filter_is_flagged(self):
        ...
        findings = self.rule.check(spec)
        assert len(findings) == 1
        assert findings[0].rule_id == "PACT-003"
    ...
    def test_mcp_fixture_returns_empty_list_without_exception(self):
        """MCP tools are normalised to POST; PACT-003 only checks DELETE/PATCH.
...
class TestIdempotencyMissing:
    ...
    def test_post_without_idempotency_header_flagged(self):
        ...
        findings = self.rule.check(spec)
        assert len(findings) == 1
        assert findings[0].rule_id == "PACT-004"
        assert findings[0].severity == "HIGH"
...
class TestSchemaVolatility:
    ...
    def test_post_with_additional_properties_true_flagged(self):
        ...
        findings = self.rule.check(spec)
        assert len(findings) == 1
        assert findings[0].rule_id == "PACT-005"
...
_ALL_RULES_SPEC: dict = {
    ...
    "paths": {
        "/items": {
            # PACT-001: api_key in query
            # PACT-002: DELETE with no path param
            # PACT-003: DELETE with no path param and no required filter
            "delete": {
            ...
            # PACT-004: POST without Idempotency-Key
            # PACT-005: requestBody with additionalProperties: true
            "post": {
                "operationId": "createItem",
                "parameters": [],
                "requestBody": {
...
class TestEngine:
    ...
    def test_openapi_fixture_finding_count_and_rule(self):
        ...
        assert all(f.rule_id == "PACT-001" for f in findings)

    def test_mcp_fixture_finding_count(self):
        # PACT-002 (CRITICAL) + PACT-004 (HIGH) + PACT-005 (MEDIUM) on delete_files
        spec = ingest(FIXTURES / "sample_mcp.json")
        findings = engine.run(spec)
        assert len(findings) == 3
    ...
    def test_all_five_rules_can_fire(self):
        ...
        assert rule_ids == {"PACT-001", "PACT-002", "PACT-003", "PACT-004", "PACT-005"}
    ...
    def test_mixed_severities_sorted_correctly(self):
        ...
        spec = _minimal_spec(**{
            "/a": {"patch": {"operationId": "patchAll", "parameters": []}},   # PACT-003 HIGH
            "/b": {"delete": {"operationId": "deleteAll", "parameters": []}},  # PACT-002 CRITICAL + PACT-003 HIGH
        })
        findings = engine.run(spec)
        positions = [_SEVERITY_ORDER[f.severity] for f in findings]
        assert positions == sorted(positions)
